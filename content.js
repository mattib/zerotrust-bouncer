console.log("ZeroTrust Bouncer POC v0.2.4: content.js loaded");
...
window.addEventListener('ZeroTrustBouncer_MapUpdate', (e) => {
    try {
        ...
        console.log("ZeroTrust Bouncer v0.2.4: Received updated PII map!", piiMap);
        unmaskNode(document.body);
    } catch (err) {
        console.error("ZeroTrust Bouncer v0.2.4: Error parsing map", err);
    }
});
    ...
    if (node.nodeType === Node.TEXT_NODE) {
        ...
        for (const [token, realValue] of Object.entries(piiMap)) {
            if (text.includes(token)) {
                ...
                modified = true;
                console.log(`ZeroTrust Bouncer v0.2.4: Unmasked ${token} on screen!`);
            }
        }
        ...
        if (mutation.type === 'childList') {
            ...
            for (const [token, realValue] of Object.entries(piiMap)) {
                if (text.includes(token)) {
                    text = text.replaceAll(token, realValue);
                    modified = true;
                    console.log(`ZeroTrust Bouncer v0.2.4: Unmasked ${token} on screen (text mutation)!`);
                }
            }
